...
- **Navbar hover system（locked，2026-09-05）**：`.pg-nav-link`——hover／active 文字 Secondary `#AFC4CF`＋1.5px 線（文字寬 55%）scaleX 由左展開＋5.5px 球沿線滑到末端右 3.5px，線與球共用 250ms ease-out（若真機看不出球在動，只把共同 duration 改 280ms，不加 delay）。色階：Logo＝Primary `#6F8FA3`（brand）／導覽預設＝首頁透明態 Sand、其他頁 ink-700／互動＝Secondary。**後續不要因其他頁修改而動它**
