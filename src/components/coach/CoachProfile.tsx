...
/**
 * 個別教練頁的內容拆成兩塊，讓桌機可以做「左教練／右預約」兩欄版面：
 *
 *   CoachSummary —— 首屏摘要：照片、角色、姓名、專長、教學理念、數據
 *   CoachDetails —— 往下延伸：關於教練、經歷、課程、場館、實戰闖關
 *
 * 手機／平板依序單欄排列（摘要 → 預約 → 詳細內容），由 InfoApp 決定順序。
 * 每個區塊都有資料才顯示，沒有資料就整段隱藏，不留空白底線。
 */

/* ------------------------------------------------------------------ */

/** 首屏摘要：這位教練是誰 */
export function CoachSummary({ coach }: { coach: Coach }) {
  ...
  return (
    <div>
      <div className="grid gap-5 sm:grid-cols-[11rem_minmax(0,1fr)] sm:gap-7">
        {/* 照片（3:4 直式；無圖時品牌漸層佔位） */}
        <div className="w-full max-w-[11rem] overflow-hidden rounded-card">
        ...
        <div className="min-w-0">
          ...
          <h1 className="mt-1.5 text-2xl sm:text-3xl">{coach.name}</h1>
          ...
          {coach.philosophy && (
            <blockquote className="mt-4 border-l-4 border-brand-600 pl-4 text-sm leading-relaxed text-ink-700 italic">
              「{coach.philosophy}」
            </blockquote>
          )}

          {socials.length > 0 && (
            <ul className="mt-3 flex flex-wrap gap-x-5 gap-y-1 text-sm text-ink-500">
      ...
      {coach.stats.length > 0 && (
        <dl className="mt-5 grid grid-cols-3 gap-2 sm:gap-3">
          {coach.stats.map((s) => (
            <div
              key={s.label}
              className="rounded-card border border-line bg-white px-2 py-3 text-center"
            >
              <dt className="sr-only">{s.label}</dt>
              <dd>
                <span className="block text-lg font-bold text-ink-900">{s.value}</span>
...
/** 首屏以下的完整內容 */
export function CoachDetails({ coach }: { coach: Coach }) {
    ...
    <div>
      ...
      {(coach.bio.length > 0 || coach.credentials.length > 0) && (
        <div className="grid gap-8 sm:grid-cols-2">
          {coach.bio.length > 0 && (
            <section>
              <h2 className="text-base font-semibold">關於教練</h2>
          ...
          {coach.credentials.length > 0 && (
            <section>
              <h2 className="text-base font-semibold">經歷與資格</h2>
      ...
      <div className="mt-10 grid gap-8 sm:grid-cols-2">
        {courses.length > 0 && (
          <section>
            <h2 className="text-base font-semibold">開設／參與的課程</h2>
        ...
        {coach.venue && (
          <section>
            <h2 className="text-base font-semibold">撞球場館地點</h2>
      ...
      {challenges.length > 0 && (
        <section className="mt-10">
          <h2 className="text-base font-semibold">對應實戰闖關</h2>
