...
- `styles/nav.css`：Navbar state architecture——`header.pg-nav[data-nav-state]`＝`transparent-light｜transparent-dark｜solid-light｜solid-dark`，`[data-nav-surface]`＝`none｜floating｜page`（floating 2026-09-05 v3＝全寬、Ivory 86% translucent＋blur 16px、1px 髮絲線、radius 0、無陰影，桌機 72px／手機 64px，token `--pg-nav-h-*`／`--pg-nav-scrolled-surface`／`--pg-nav-blur`／`--pg-nav-hairline`；**不要再做 pill 浮板或實心橫條**）；顏色全走 `--nav-fg／--nav-fg-hover／--nav-indicator／--nav-logo／--nav-icon／--nav-burger`。Logo 品牌字讀 `--logo-color`（`.pg-logo` Primary、`.pg-logo--on-dark` 白、Navbar 內接 `--nav-logo`）。觸發鈕加 `.pg-nav-trigger` 才吃 nav 顏色，popover／手機選單自己白底深字，**不再有 revert-layer**
