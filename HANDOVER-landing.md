...
> **2026-09-16 晚更新**：首頁 IA 重組（雙入口／App 預覽／教練卡 v3）、`/app` 玩法頁、教練列表頁、教練詳細頁與示範預約流程全部完成於本機，約 40 個 commit 未 push。
> 使用者指示：**先不推，等他改好再一次推**。推的時候：部署 repo 要多產根目錄 `coach-profile.html`、`app-play.html`，並新增 `vercel.json`
> `{"rewrites":[{"source":"/app","destination":"/app-play.html"}]}`。已定案：底色 01 淺霧藍／02 米白、預約面板底 `#FDFCF9`、主色 `#2B66B4`。
> 待補素材：LEARN 入口「教練指導學生」照片（現用場館圖並標示意）、三位真實教練資料。
