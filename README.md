...
| 網址 | 頁面 | 說明 |
|---|---|---|
| `/` | 首頁 | 五區塊：Hero、痛點、功能卡、對照圖、CTA |
| `/about.html` | 關於 Poolgress | 品牌長期願景 |
...
個別教練頁最下方有預約行事曆，資料在 `src/data/coaches.ts` 每位教練的 `availability`：

```ts
availability: {
  '2026-09-03': ['19:00', '20:00'],   // 日期 → 當天開放的時間
  '2026-09-05': ['14:00'],
}
```

- 有列出的日期會在月曆上顯示**綠色勾勾**，可以點選
- 留 `{}`（空物件）＝這位教練不開放預約，該頁不顯示行事曆
- 目前四位教練用的是 `demoAvailability()` **示範資料**（每週二、四、六），
  上線前必須換成真實開放時段
- 單堂費用填在同一位教練的 `lessonPrice`（數字，如 `1200`）；
  留 `null` 付款畫面會顯示「NT$＿＿（待確認）」
- 預約流程：選時段 → 付款畫面（填 Email、選付款方式）→ 付款成功 → 預約成功
...
| 教練預約 | 開放時段為示範產生、費用為 null；付款與確認信皆未串接 | 填真實時段與費用，接排程／金流／寄信 |
